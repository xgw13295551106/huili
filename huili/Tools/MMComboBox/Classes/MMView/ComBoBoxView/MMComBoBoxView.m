...
- (void)didTapDropDownBox:(MMDropDownBox *)dropDownBox atIndex:(NSUInteger)index {
    if (index==2) {
        ...
        MMDropDownBox *currentBox  = self.dropDownBoxArray[2];
        [currentBox updateTitleState:YES];
        if ([self.delegate respondsToSelector:@selector(showSelectView:)]) {
            [self.delegate showSelectView:1];
        }
        ...
        if ([self.delegate respondsToSelector:@selector(showSelectView:)]) {
            [self.delegate showSelectView:2];
        }
    }
    ...
    if (self.symbolArray.count) {
        ...
        CGRect fram=CGRectMake(self.frame.origin.x, self.frame.origin.y+64, self.frame.size.width, self.frame.size.height);
        [popupView popupViewFromSourceFrame:fram completion:^{
           self.isAnimation = NO;
        }];
