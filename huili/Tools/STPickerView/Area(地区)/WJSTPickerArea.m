...
@interface WJSTPickerArea()<UIPickerViewDataSource, UIPickerViewDelegate>
...
- (void)setupUI
{
    ...
    NSMutableArray *citys = [NSMutableArray arrayWithArray:[self.arrayRoot firstObject][@"sub"]];
    [citys enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [self.arrayCity addObject:obj[@"name"]];
    }];

    self.arrayArea = [citys firstObject][@"sub"];
...
{
    if (component == 0) {
        self.arraySelected = self.arrayRoot[row][@"sub"];
        ...
        self.arrayArea = [NSMutableArray arrayWithArray:[self.arraySelected firstObject][@"sub"]];
        ...
        if (self.arraySelected.count == 0) {
            self.arraySelected = [self.arrayRoot firstObject][@"sub"];
        }

        self.arrayArea = [NSMutableArray arrayWithArray:[self.arraySelected objectAtIndex:row][@"sub"]];
